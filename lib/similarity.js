...
const COLOUR_FAMILIES = {
  light: ['white', 'ivory', 'cream', 'beige', 'oat', 'oatmeal', 'natural', 'linen', 'off-white', 'sand', 'bone', 'ecru', 'snow', 'opal white', 'opal', 'oyster'],
  dark: ['black', 'charcoal', 'ebony', 'onyx', 'ink'],
  grey: ['grey', 'gray', 'silver', 'smoke', 'slate'],
  timber: ['oak', 'walnut', 'tan', 'brown', 'chestnut', 'caramel', 'honey', 'natural oak', 'chocolate', 'timber'],
...
// Colours get plain lowercase/trim only - normaliseToken is a MATERIALS
// helper and would rewrite the common AU colourway "timber" to "wood",
// silently zeroing its family credit against oak/walnut/tan.
function colourToken(c) {
  return String(c || '').toLowerCase().trim();
}

function colourFamily(c) {
  const t = colourToken(c);
  if (!t) return null;
...
function colourScore(a, b) {
  const ta = colourToken(a);
  const tb = colourToken(b);
...
function similarity(original, candidate) {
  ...
  // Honesty cap: without comparable dimensions a pair can look identical on
  // paper-thin data, so it can never claim the top band.
  if (components.dimensions === null) score = Math.min(score, 89);
...
// ── self-test ────────────────────────────────────────────────────────────────
// typeof-guarded: this file is imported by api/og.js's Vercel Edge Function,
// where `require` isn't a global at all - referencing it unguarded throws
// (ReferenceError) the instant this module loads, before the self-test
// condition can even short-circuit to false. Caught live on first deploy.
if (typeof require !== 'undefined' && require.main === module && process.argv.includes('--test')) {
  ...
  assert.ok(noDims.score <= 89, `missing dims must cap at 89, got ${noDims.score}`);
  const perfectNoDims = similarity(orig, { ...orig, price_aud: 999, dimensions_cm: undefined });
  assert.ok(perfectNoDims.score <= 89, 'even a perfect no-dims pair stays capped');
  ...
  assert.strictEqual(colourScore('timber', 'oak'), 0.7, 'timber colourway must keep its family credit');
  assert.strictEqual(colourScore('Timber ', 'walnut'), 0.7, 'colour tokens trim + lowercase');
