  ...
  (function () {
    ...
    chips.forEach(function (c) {
      c.addEventListener('click', function () {
        ...
        var cap = Number(c.getAttribute('data-cap') || 0);
        document.querySelectorAll('.card').forEach(function (card) {
          var p = Number(card.getAttribute('data-price') || 0);
          card.classList.toggle('hide', cap > 0 && p > cap);
        });
...
function buildIndex(catalogue, matchMap, env) {
  ...
  const sections = Object.keys(CATEGORY_LABELS).map((cat) => {
    ...
    const cards = inCat.map((o) => {
      ...
      return `<a class="card reveal" data-price="${alt ? Number(alt.price_aud) : 0}" href="compare/${esc(slugify(o.id))}.html">
        <div class="card-head">
  ...
  <div class="chipbar reveal wrap" role="group" aria-label="Filter by top-match price">
