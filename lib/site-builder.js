...
// Product photo: hotlinked straight from the retailer's own hosted URL
// (never downloaded or re-hosted by us - see AFFILIATES.md "Image sourcing").
// If a product has no recorded image yet, or the hotlinked URL breaks later,
// this degrades to a plain placeholder box rather than a broken-image icon.
function photoTag(imageUrl, altText, extraClass) {
  const cls = `thumb${extraClass ? ' ' + extraClass : ''}`;
  if (!imageUrl) return `<span class="${cls} thumb-empty" aria-hidden="true"></span>`;
  return `<span class="${cls}"><img src="${esc(imageUrl)}" alt="${esc(altText)}" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="this.parentElement.classList.add('thumb-empty');this.remove()"></span>`;
}
...
const LEGAL_FOOTER = `
  <footer class="footer">
    <div class="wrap">
      ...
      with the original brand. All trademarks belong to their owners. Product photos are linked directly from
      each retailer's own listing, not copied or altered. Prices are checked periodically and may have changed
      &mdash; always confirm on the retailer's site. Some outbound links may earn us a commission at no cost
      to you.</p>
...
// The OG card is a separate Vercel Edge Function (api/og.js), not part of
// this static build - GitHub Pages can't run it. Omitted entirely (no
// og:image tag) until OG_IMAGE_BASE_URL is configured, same pattern as
// SITE_URL/canonical below.
function ogImageUrl(env, id) {
  const base = strEnv('OG_IMAGE_BASE_URL', env);
  if (!base) return '';
  return id ? `${base}/api/og?id=${encodeURIComponent(id)}` : `${base}/api/og`;
}

function page({ title, description, canonical, body, root = '', ogImage = '' }) {
  return `<!DOCTYPE html>
<html lang="en-AU">
<head>
  <meta charset="UTF-8">
  ...
  const sections = Object.keys(CATEGORY_LABELS).map((cat) => {
    ...
    const cards = inCat.map((o) => {
      ...
      return `<a class="card reveal" data-price="${alt ? Number(alt.price_aud) : 0}" data-search="${esc(haystack)}" href="compare/${esc(slugify(o.id))}.html">
        ${photoTag(o.image_url, `${o.brand || ''} ${o.name}`, 'thumb-card')}
        <div class="card-head">
...
function buildComparePage(original, matches, catalogue, env) {
  ...
  const rows = matches.map((m, i) => {
    ...
    return `<article class="alt-row reveal" style="transition-delay:${i * 60}ms">
      ...
      <div class="alt-body">
        <div class="alt-top">
          ${photoTag(alt.image_url, `${alt.retailer || ''} ${alt.name}`, 'thumb-alt')}
          <div class="alt-top-text">
  ...
  <main class="wrap main">
    ...
    <section class="orig-panel reveal">
      ...
      <div class="orig-grid">
        <div class="orig-top">
          ${photoTag(original.image_url, `${original.brand || ''} ${original.name}`, 'thumb-hero')}
          <div>
...
${rows}
  ...
  return page({
    ...
    body,
    root: '../',
    ogImage: ogImageUrl(env, original.id),
  });
}
...
/* Product photos: hotlinked from the retailer's own hosted URL - see
   AFFILIATES.md "Image sourcing". thumb-empty is the graceful fallback for
   products with no recorded image, or if a hotlinked URL breaks later. */
.thumb { display: block; background: var(--panel-2); flex: none; }
.thumb img { width: 100%; height: 100%; object-fit: cover; display: block; }
.thumb-empty { background: repeating-linear-gradient(135deg, var(--panel-2), var(--panel-2) 6px, var(--panel) 6px, var(--panel) 12px); }
.thumb-card { width: calc(100% + 40px); height: 170px; margin: -20px -20px 4px; border-bottom: 1px solid var(--line); }
.thumb-hero { width: 168px; height: 168px; border-radius: 10px; border: 1px solid var(--line); }
.thumb-alt { width: 76px; height: 76px; border-radius: 8px; border: 1px solid var(--line); }
...
.orig-top { display: flex; gap: 20px; align-items: flex-start; }
...
.alt-top { display: flex; gap: 14px; align-items: flex-start; margin-bottom: 10px; }
.alt-top-text { flex: 1; min-width: 0; }
...
@media (max-width: 800px) {
  ...
  .thumb-hero { width: 96px; height: 96px; }
  .thumb-alt { width: 56px; height: 56px; }
}
`;
