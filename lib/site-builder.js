...
function buildMethodology(env) {
  ...
  <main class="wrap main method">
    ...
    <section class="method-block reveal">
      ...
      publish an attribute, the score reweights across what is available rather than guessing. Products in
      different categories are never compared.</p>
    </section>
    <section class="method-block reveal">
      <h2>Bands</h2>
...
function buildSite(catalogue, matches, outDir, env = process.env) {
  ...
  fs.writeFileSync(path.join(outDir, '404.html'), page({
    ...
    body: '<main class="wrap main"><h1 class="orig-name">404 &mdash; not found</h1><p class="hero-sub" style="margin-top:12px"><a class="text-link" href="index.html">Back to the index</a></p></main>',
  }));
